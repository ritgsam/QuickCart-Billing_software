...
<form action="{{ isset($invoice) ? route('sale_invoices.update', $invoice->id) : route('sale_invoices.store') }}" method="POST">
...
<input type="date" name="invoice_date" class="w-full p-2 border rounded" value="{{ old('invoice_date', $invoice->invoice_date ?? '') }}">
...
<tbody>
    <tr class="product-row">
        ...
        <td class="px-4 py-2 border"><input type="number" name="products[0][quantity]" class="quantity w-full p-2 border rounded" value="1" required></td>
...
<td class="px-4 py-2 border">
    <select name="products[0][gst_rate]" class="gst-rate w-full p-2 border rounded" onchange="updateTotal()">
        ...
        <td class="px-4 py-2 border"><input type="number" name="products[0][discount]" class="discount w-full p-2 border rounded" value="0"></td>
...
document.querySelectorAll('.product-select, .quantity, .gst-rate, .discount').forEach(input => {
    input.addEventListener('input', updateTotal);
});

document.querySelector('input[name="global_discount"]').addEventListener('input', function () {
    applyGlobalDiscount(parseFloat(document.getElementById('total-amount').value) || 0);
});
</script>
